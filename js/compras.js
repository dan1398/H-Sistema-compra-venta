...
function agregarCarrito(parametro) {
    var elemento = {
        producto: productos[parametro].producto1, 
    ...
    for (let i = 0; i < carrito.length; i++) {
        cadena += `
            <tr>
                ...
                <td>
                    <input type="number" onchange="cambiaCantidad(${i}, this)" value="${carrito[i].cantidad}" class="form">
                </td>
...
function registrarCompra() {
    ...
    for (let i = 0; i < carrito.length; i++) {
        for (let j = 0; j < losProductos.length; j++) {
            if (losProductos[j].producto1 == carrito[i].producto) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            for (let i = 0; i < laCompra.detalles.length; i++) {
                for (let j = 0; j < losProductos.length; j++) {
                    if (losProductos[j].producto1 == laCompra.detalles[i].producto) {
...
function mostrarCompra(){
    ...
    if(laCompra == undefined || laCompra == null){
        Swal.fire({
            ...
            text: 'La compra no existe o a sido eliminada',
            icon: 'warning',
        }).then((result) => {
            window.location.href = "compras.html";
        });
